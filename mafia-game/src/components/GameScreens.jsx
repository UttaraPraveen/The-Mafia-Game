    ...
    return (
      <div className="screen">
        ...
        <div style={{ width: "100%", maxWidth: 420, animation: "fadeUp 0.5s ease forwards" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {new Set(playerNames).size !== playerNames.length && <p style={{ color: "var(--blood)", fontSize: "0.72rem", textAlign: "center", letterSpacing: "0.1em", animation: "startle 0.5s ease" }}>Players must use unique identifiers.</p>}
            <button className="btn-primary" onClick={handleStart} disabled={!isValid || playerNames.some(n => !n.trim()) || new Set(playerNames).size !== playerNames.length}><span>BEGIN THE NIGHT ✦</span></button>
